...
export function AuthShell({
  ...
  return (
    <main className="min-h-screen bg-[#f8f5ef] text-foreground lg:grid lg:grid-cols-[minmax(0,0.9fr)_minmax(560px,1.1fr)]">
      <section className="relative hidden min-h-screen overflow-hidden bg-[#1d1033] px-10 py-9 text-white lg:flex lg:flex-col xl:px-16 xl:py-12">
        ...
        <div className="relative z-10 my-auto max-w-xl py-16">
          <p className="text-sm font-semibold text-[#f4bd62]">One connected bakery workspace</p>
          ...
          <div className="mt-10 space-y-4">
            {benefits.map((benefit) => (
              <div key={benefit} className="flex items-center gap-3 text-sm font-semibold text-white/85">
                <span className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full border border-white/15 bg-white/10 text-[#f4bd62]">
        ...
        <div className="relative z-10 flex items-center gap-3 border-t border-white/10 pt-6 text-sm text-white/60">
          <MessageCircle className="h-4 w-4 text-[#f4bd62]" />
