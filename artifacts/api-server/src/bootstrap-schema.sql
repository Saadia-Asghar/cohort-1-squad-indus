...
ALTER TABLE sweet_tooth.orders ADD COLUMN IF NOT EXISTS deposit_required_pkr INTEGER;
ALTER TABLE sweet_tooth.orders ADD COLUMN IF NOT EXISTS quote_expires_at TIMESTAMPTZ;
ALTER TABLE sweet_tooth.orders ADD COLUMN IF NOT EXISTS customer_approved_at TIMESTAMPTZ;
ALTER TABLE sweet_tooth.orders ADD COLUMN IF NOT EXISTS customer_rejected_at TIMESTAMPTZ;
...
FROM (VALUES
  ...
  ('fatima-cakery', 'Fondant Wedding Cake', 'Elegant custom wedding cakes with sugar flowers.', 15000, '[{"label":"2 Tier","pricePkr":15000}]', '{}', true, 7, 'Cakes', '{Wedding,Nikah}', '{}', 'https://images.unsplash.com/photo-1464349095431-e9a21285b5f3?w=600&auto=format&fit=crop', 34, true, true, 1),
